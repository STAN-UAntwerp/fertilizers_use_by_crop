...
class HistGradientBoostEstimator(Estimator):
    configcls = Config
    abbrev = 'HGB'
    ...
    def preprocess(self, dataset: DataSet) -> DataSet:
        ...
        # Histgradientboost regressor is trained with CV, so we can combine train and validation set
    ...
    def fit(self, dataset: DataSet) -> HistGradientBoostEstimator:
        logger.debug(f"Training model.")
        hyperparam = self.config.dict()
        if self.cv:
            hyperparam['cv'] = self.cv

        self.grid = GridSearchCV(HistGradientBoostingRegressor(), **hyperparam, scoring='neg_mean_absolute_error') 
    ...
    def get_feature_importance(self, estimator_name, dataset_x: pd.DataFrame, it: int=0, plot_: bool=True, shap_: bool=False, perm_: bool=False) -> tuple:
    ...
    def get_evaluation_metrics(self, dataset: DataSet, set: str) -> pd.Series:
        if set=='test':
            ...
            y_pred = pd.Series(self.model.predict(dataset.X_train), index=dataset.X_train.index,
                                     name='predicted_' + data_config.target)
        return get_evaluation_metrics_reg(y, y_pred).rename('histgradientboost')
    ...
    @classmethod
    def load(cls, path: str | Path, it: int=0) -> HistGradientBoostEstimator:
        model = joblib.load(path / f'model_{it}.pkl')
        estimator = HistGradientBoostEstimator()
